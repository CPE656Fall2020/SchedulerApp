...
namespace SchedulerGUI.Solver
{
    ...
    public interface IScheduleSolver
    {
        ...
        /// <param name="summarizedEncryptors">The byte processing encryption profiles describing the summarized devices that are available to schedule for.</param>
        /// <param name="summarizedCompressors">The byte processing compression profiles describing the summarized devices that are available to schedule for.</param>
        ...
        ScheduleSolution Solve(IEnumerable<PassOrbit> passes, IEnumerable<IByteStreamProcessor> summarizedEncryptors, IEnumerable<IByteStreamProcessor> summarizedCompressors, Battery battery);
    }
}
